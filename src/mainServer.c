...
void generateIPs(char *target, char *mask, char **ips)
{
    ...
    for (ip = network + 1; ip < broadcast; ip++)
    {
        ...
        if (inet_ntop(AF_INET, &addr, str, INET_ADDRSTRLEN) == NULL) {
            // Gestion de l'erreur de conversion d'adresse IP
            continue;
        }
        str[INET_ADDRSTRLEN - 1] = '\0'; // Ajout du caractère nul
        if (index < 256)
        {
...
int main()
{
    ...
    while (1)
    {
        ...
        {
            send(dialogSocket, "Available commands:\n- help\n- scan: [0-1](default 0) -targetV (default localhost)\n- exit\nCheck README.md for more info.", 200, 0);
        }
        ...
        else if (prefix("scan", buffer) && buffer[strlen(buffer) - 1] != '\0')
        {
            ...
            if (scanType == 0)
            {
                ...
                if (result.s_addr == target.s_addr)
                {
                    ...
                    char** ips = malloc(256 * sizeof(char*));
                    int* ipsCount = malloc(sizeof(int));
                    horizontalScan(scanHTarget, scanHMask, ips, ipsCount);

                    char sendBuffer[200];
                    for(int i = 0; i<(*ipsCount); i++){
                        snprintf(sendBuffer, sizeof(sendBuffer), "Device found: \033[0;31m%s\033[0;37m\n", ips[i]);
                        send(dialogSocket, ips[i], 200, 0);
                    }
                    printf("Horizontal scan done\n");
                    ...
                    send(dialogSocket, "Horizontal scan done\n", 200, 0);
